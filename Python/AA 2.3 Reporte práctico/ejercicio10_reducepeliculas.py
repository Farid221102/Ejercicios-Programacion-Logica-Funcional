...
peliculasFiltro = list(filter(lambda pelicula: pelicula["Director"] == "Christopher Nolan", peliculasXD))

peliculasMap = list(map(lambda pelicula: {"titulo": pelicula["Titulo"],"calificacion": pelicula["CalificaciónIMDB"]}, peliculasFiltro))

suma_edades = reduce(lambda acumulador, pelicula: acumulador + float(pelicula["calificacion"]), peliculasMap, 0)

promedio = suma_edades / len(peliculasMap)

print(peliculasMap)
print("")
print("Suma de calificaciones:", suma_edades)
